...
class Config:
    ...
    GRAD_CLIP = 1.0  # 梯度裁剪值
    LOSS_FUNCTION = nn.L1Loss()  # 更换损失函数
    ...
    def generate_training_data(self):
        ...
        for _ in range(Config.TRAIN_SAMPLES):
            ...
            node_features = torch.tensor(
                self.df.iloc[np.concatenate([team, enemy])].drop(['original_type1', 'original_type2'], axis=1).values,
                dtype=torch.float32
            )
    ...
    def train(self):
        ...
        for epoch in range(Config.TRAIN_EPOCHS):
            ...
            for batch in loader:
                ...
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), Config.GRAD_CLIP)  # 梯度裁剪
                self.optimizer.step()
...
class GeneticOptimizer:
    ...
    def _create_graph_data(self, team):
        combined_indices = team + self.enemy_team
        # 检查索引是否有效
        for idx in combined_indices:
            if idx not in self.df.index:
                print(f"Invalid index in combined_indices: {idx}")
        node_features = torch.tensor(
            self.df.iloc[combined_indices].drop(['original_type1', 'original_type2'], axis=1).values,
            dtype=torch.float32
        )
        edge_index = torch.combinations(torch.arange(node_features.size(0)), 2).t()
        data = Data(
